...
static const int BlockedStorm[8][2] = {{0, 0},   {0, 0},  {76, 78}, {-10, 15},
                                       {-7, 10}, {-4, 6}, {-1, 2}};
...
static const Value UnblockedStorm[4][8] = {{V(85), V(-289), V(-166), V(97), V(50), V(45), V(50)},
...
INLINE Score pawn_evaluate(const Position* pos, PawnEntry* e, const Color Us) {
    ...
    e->blockedCount += popcount(shift_bb(Up, ourPawns) & (theirPawns | doubleAttackThem));
    ...
    loop_through_pieces(Us, PAWN, s) {
        ...
        backward = !(neighbours & forward_ranks_bb(Them, rank_of(s + Up))) && (leverPush | blocked);
        ...
        if (support | phalanx)
        {
            int v = Connected[r] * (2 + !!phalanx - !!opposed) + 21 * popcount(support);
        ...
        if (!support)
            score -= (doubled ? Doubled : 0) + (more_than_one(lever) ? WeakLever : 0);
...
INLINE void evaluate_shelter(
  const PawnEntry* pe, const Position* pos, Square ksq, Score* shelter, const Color Us) {
    const Color Them = (Us == WHITE ? BLACK : WHITE);
...
INLINE Score do_king_safety(PawnEntry* pe, const Position* pos, Square ksq, const Color Us) {
    ...
    if (!pawns)
        ...
        for (minPawnDist = 1; minPawnDist < 6 && !(DistanceRingBB[ksq][minPawnDist] & pawns);
             minPawnDist++)
            ;
...
NOINLINE Score do_king_safety_white(PawnEntry* pe, const Position* pos, Square ksq) {
    return do_king_safety(pe, pos, ksq, WHITE);
}

NOINLINE Score do_king_safety_black(PawnEntry* pe, const Position* pos, Square ksq) {
