...
static const Score MobilityBonus[4][32] = {
  // Knight
  {S(-62, -81), S(-53, -56), S(-12, -31), S(-4, -16), S(3, 5), S(13, 11), S(22, 17), S(28, 20),
   S(33, 25)},
  // Bishop
  {S(-48, -59), S(-20, -23), S(16, -3), S(26, 13), S(38, 24), S(51, 42), S(55, 54), S(63, 57),
   S(63, 65), S(68, 73), S(81, 78), S(81, 86), S(91, 88), S(98, 97)},
  // Rook
  {S(-60, -78), S(-20, -17), S(2, 23), S(3, 39), S(3, 70), S(11, 99),  // Rook
   S(22, 103), S(31, 121), S(40, 134), S(40, 139), S(41, 158), S(48, 164), S(57, 168), S(57, 169),
   S(62, 172)},
  // Queen
...
static const Score ThreatByMinor[8] = {S(0, 0),   S(5, 32),   S(55, 41),
                                       S(77, 56), S(89, 119), S(79, 162)};

static const Score ThreatByRook[8] = {S(0, 0), S(3, 44), S(37, 68), S(42, 60), S(0, 39), S(58, 43)};
...
// independently.
static const Value PassedRank[][8] = {{V(0), V(9), V(15), V(17), V(64), V(171), V(277)},
                                      {V(0), V(28), V(31), V(39), V(70), V(177), V(260)}};

// PassedFile[File] contains a bonus according to the file of a passed pawn
static const Score PassedFile[8] = {S(0, 0),   S(11, 8),  S(22, 16), S(33, 24),
...
INLINE void evalinfo_init(const Position* pos, EvalInfo* ei, const Color Us) {
    ...
    // mobility area
    ei->mobilityArea[Us] =
      ~(b | pieces_cpp(Us, KING, QUEEN) | blockers_for_king(pos, Us) | ei->pe->pawnAttacks[Them]);
    ...
    Square s =
      make_square(clamp(file_of(ksq), FILE_B, FILE_G), clamp(rank_of(ksq), RANK_2, RANK_7));
...
INLINE Score
evaluate_pieces(const Position* pos, EvalInfo* ei, Score* mobility, const Color Us, const int Pt) {
    const Color    Them = (Us == WHITE ? BLACK : WHITE);
    const int      Down = (Us == WHITE ? SOUTH : NORTH);
    ...
    loop_through_pieces(Us, Pt, s) {
        ...
        b = Pt == BISHOP ? attacks_bb_bishop(s, pieces() ^ pieces_p(QUEEN))
          : Pt == ROOK   ? attacks_bb_rook(s, pieces() ^ pieces_p(QUEEN) ^ pieces_cp(Us, ROOK))
                         : attacks_from(Pt, s);
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            if (Pt == KNIGHT && (bb & sq_bb(s) & ~CenterFiles)  // on a side outpost
            ...
            if (Pt == BISHOP)
            {
                ...
                score -=
                  BishopXRayPawns * popcount(PseudoAttacks[BISHOP][s] & pieces_cp(Them, PAWN));
...
INLINE Score evaluate_king(const Position* pos, EvalInfo* ei, Score* mobility, const Color Us) {
    const Color    Them = Us == WHITE ? BLACK : WHITE;
    ...
    Score score =
      Us == WHITE ? king_safety_white(ei->pe, pos, ksq) : king_safety_black(ei->pe, pos, ksq);

    // Attacked squares defended at most once by our queen or king
    weak = ei->attackedBy[Them][0] & ~ei->attackedBy2[Us]
         & (~ei->attackedBy[Us][0] | ei->attackedBy[Us][KING] | ei->attackedBy[Us][QUEEN]);
    ...
    if (rookChecks)
        kingDanger += more_than_one(rookChecks) ? RookSafeCheck * 175 / 100 : RookSafeCheck;
    else
        unsafeChecks |= b1 & ei->attackedBy[Them][ROOK];

    queenChecks =
      (b1 | b2) & ei->attackedBy[Them][QUEEN] & safe & ~(ei->attackedBy[Us][QUEEN] | rookChecks);
    if (queenChecks)
        kingDanger += more_than_one(queenChecks) ? QueenSafeCheck * 145 / 100 : QueenSafeCheck;

    // Enemy bishops checks: we count them only if they are from squares from
    // which we can't give a queen check, because queen checks are more valuable.
    bishopChecks = b2 & ei->attackedBy[Them][BISHOP] & safe & ~queenChecks;
    if (bishopChecks)
        kingDanger += more_than_one(bishopChecks) ? BishopSafeCheck * 3 / 2 : BishopSafeCheck;
    else
    ...
    if (knightChecks & safe)
        kingDanger +=
          more_than_one(knightChecks & safe) ? KnightSafeCheck * 162 / 100 : KnightSafeCheck;
    else
    ...
    kingDanger += ei->kingAttackersCount[Them] * ei->kingAttackersWeight[Them]
                + 185 * popcount(ei->kingRing[Us] & weak) + 148 * popcount(unsafeChecks)
                + 98 * popcount(blockers_for_king(pos, Us)) + 69 * ei->kingAttacksCount[Them]
                + 3 * kingFlankAttack * kingFlankAttack / 8
                + mg_value(mobility[Them] - mobility[Us]) - 873 * !pieces_cp(Them, QUEEN)
                - 100 * !!(ei->attackedBy[Us][KNIGHT] & ei->attackedBy[Us][KING])
                - 6 * mg_value(score) / 8 - 4 * kingFlankDefense + 37;
...
INLINE Score evaluate_threats(const Position* pos, EvalInfo* ei, const Color Us) {
    ...
    stronglyProtected = ei->attackedBy[Them][PAWN] | (ei->attackedBy2[Them] & ~ei->attackedBy2[Us]);
    ...
    if (defended | weak)
    {
        ...
        b = ~ei->attackedBy[Them][0] | (nonPawnEnemies & ei->attackedBy2[Us]);
    ...
    b = ei->attackedBy[Them][0] & ~stronglyProtected & ei->attackedBy[Us][0];
    ...
    if (piece_count(Them, QUEEN) == 1)
    {
        ...
        safe     = ei->mobilityArea[Us] & ~pieces_cp(Us, PAWN) & ~stronglyProtected;
...
// Helper function
INLINE int capped_distance(Square s1, Square s2) { return min(distance(s1, s2), 5); }
...
INLINE Score evaluate_passed(const Position* pos, EvalInfo* ei, const Color Us) {
    ...
    if (blockedPassers)
    {
        helpers = shift_bb(Up, pieces_cp(Us, PAWN)) & ~pieces_c(Them)
                & (~ei->attackedBy2[Them] | ei->attackedBy[Us][0]);

        // Remove blocked candidate passers that don't have help to pass
        b &= ~blockedPassers | shift_bb(WEST, helpers) | shift_bb(EAST, helpers);
    }
    ...
    while (b)
    {
        ...
        if (r > RANK_3)
        {
            ...
            ebonus += ((capped_distance(square_of(Them, KING), blockSq) * 19) / 4
                       - capped_distance(square_of(Us, KING), blockSq) * 2)
                    * w;
...
INLINE Score evaluate_space(const Position* pos, EvalInfo* ei, const Color Us) {
    ...
    Bitboard safe = SpaceMask & ~pieces_cp(Us, PAWN) & ~ei->attackedBy[Them][PAWN];
...
INLINE Value evaluate_winnable(const Position* pos, EvalInfo* ei, Score score) {
    ...
    bool pawnsOnBothFlanks = (pieces_p(PAWN) & QueenSide) && (pieces_p(PAWN) & KingSide);

    bool almostUnwinnable = outflanking < 0 && !pawnsOnBothFlanks;

    bool infiltration =
      rank_of(square_of(WHITE, KING)) > RANK_4 || rank_of(square_of(BLACK, KING)) < RANK_5;

    // Compute the complexity bonus for the attacking side
    int complexity = 9 * ei->pe->passedCount + 12 * popcount(pieces_p(PAWN)) + 9 * outflanking
                   + 21 * pawnsOnBothFlanks + 24 * infiltration + 51 * !non_pawn_material()
        ...
        if (opposite_bishops(pos))
        {
            if (non_pawn_material_c(WHITE) == BishopValueMg
                ...
                sf = 22 + 3 * popcount(pieces_c(strongSide));
                 ...
                 && !(KingSide & pieces_cp(strongSide, PAWN))
                      != !(QueenSide & pieces_cp(strongSide, PAWN))
                 && (attacks_from_king(square_of(!strongSide, KING))
                     & pieces_cp(!strongSide, PAWN)))
            sf = 36;
        else if (popcount(pieces_p(QUEEN)) == 1)
            sf = 37
               + 3
                   * (pieces_cp(WHITE, QUEEN)
                        ? piece_count(BLACK, BISHOP) + piece_count(BLACK, KNIGHT)
    ...
    v =
      mg * ei->me->gamePhase + eg * (PHASE_MIDGAME - ei->me->gamePhase) * sf / SCALE_FACTOR_NORMAL;
    v /= PHASE_MIDGAME;
...
#define lazy_skip(v) (abs(mg_value(score) + eg_value(score)) / 2 > v + non_pawn_material() / 64)
    ...
    score += evaluate_king(pos, &ei, mobility, WHITE) - evaluate_king(pos, &ei, mobility, BLACK);

    // Evaluate passed pawns, we need full attack information including king
    score += evaluate_passed(pos, &ei, WHITE) - evaluate_passed(pos, &ei, BLACK);

    if (lazy_skip(LazyThreshold2))
        goto make_v;

    // Evaluate tactical threats, we need full attack information including king
    score += evaluate_threats(pos, &ei, WHITE) - evaluate_threats(pos, &ei, BLACK);

    // Evaluate space for both sides, only during opening
    score += evaluate_space(pos, &ei, WHITE) - evaluate_space(pos, &ei, BLACK);
