...
static void score_quiets(const Position* pos) {
    ...
    for (ExtMove* m = st->cur; m < st->endMoves; m++)
    {
        ...
        m->value = (*history)[us][move] + 2 * (*cmh)[pc][to] + 2 * (*fmh)[pc][to]
                 + 2 * (*fmh2)[pc][to] + (*fmh3)[pc][to]
                 + (st->mp_ply < MAX_LPH ? min(4, st->depth / 3) * (*lph)[st->mp_ply][move] : 0);

        // Bonus for checks
        m->value += ((bool)(st->checkSquares[pt] & to)) * 16384;

        // Bonus for escaping from capture
        if (threatenedPieces & square_bb(from))
        {
            m->value += (pt == QUEEN && !(square_bb(to) & threatenedByRook)) ? 51700
                      : (pt == ROOK && !(square_bb(to) & threatenedByMinor)) ? 25600
                      : !(square_bb(to) & threatenedByPawn)                  ? 14450
                                                                             : 0;
        }
        ...
        if (pt == QUEEN)
        {
            m->value -= ((bool)(square_bb(to) & threatenedByRook)) * 49000;
        }
        else if (pt == ROOK)
        {
            m->value -= ((bool)(square_bb(to) & threatenedByMinor)) * 24335;
        }
    }
}
