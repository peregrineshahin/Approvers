...
SMALL static void score_quiets(const Position* pos) {
    ...
    for (ExtMove* m = st->cur; m < st->endMoves; m++)
    {
        ...
        m->value = (mp_v4 * (*history)[attacked_by_opp(pos, from_sq(move))][c][move]
                    + mp_v5 * (*contHist0)[pt][to] + mp_v6 * (*contHist1)[pt][to]
...
static void score_evasions(const Position* pos) {
    ...
    for (ExtMove* m = st->cur; m < st->endMoves; m++)
        if (capture_stage(pos, m->move))
            ...
            m->value = (*history)[attacked_by_opp(pos, from_sq(m->move))][c][from_to(m->move)]
