...
void setoption(char* str) {
    ...
    if (strstr(name, "l1w3_v"))
    {
        int i            = atoi(name + 6);
        l1_weights[3][i] = atoi(value);
        return;
    }
    ...
    while (get_input(cmd))
    {
        ...
        else if (strcmp(token, "uci") == 0)
        {
            ...
            for (int i = 0; i < L1SIZE; i++)
                printf("option name ftb_v%d type string\n", i);

            for (int i = 0; i < L1SIZE * 2; i++)
                printf("option name l1w3_v%d type string\n", i);
        ...
        {
            for (int i = 0; i < L1SIZE; i++)
                printf("ftb_v%d, int, %d, -127, 127, %.3f, 0.002\n", i, in_biases[i],
                       abs(in_biases[i]) / 20.0);

            for (int i = 0; i < L1SIZE * 2; i++)
                printf("l1w3_v%d, int, %d, -127, 127, %.3f, 0.002\n", i, l1_weights[3][i],
                       abs(l1_weights[3][i]) / 20.0);
        }
